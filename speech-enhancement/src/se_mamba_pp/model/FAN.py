...
class FANFFN_gate_freq(nn.Module):
    """
    FANLayer: The layer used in FAN (https://arxiv.org/abs/2410.02675).
    code: https://github.com/YihongDong/FAN/blob/main/FANLayer.py
    Args:
    ...
    def __init__(self, input_dim, expansion):
        super(FANFFN_gate_freq, self).__init__()
        ...
        self.FAN1 = FANLayer(self.input_dim, self.expansion_dim, p_ratio=0.25, activation='gelu', use_p_bias=True)
        self.FAN2 = FANLayer(self.expansion_dim, self.expansion_dim, p_ratio=0.25, activation='gelu', use_p_bias=True)
        self.Linear = nn.Linear(self.expansion_dim, self.input_dim*2)

    def forward(self, src):
        # src.shape = B, C, T, F
        src = F.gelu(src)
        #x = self.layernorm(src.permute(0, 2, 3, 1))
        x = self.FAN1(src) # [B, C, T, expansion*F]
        x = self.FAN2(x) # [B, C, T, expansion*F]
        output = self.Linear(x) # [B, C, T, 2F]
        output1, output2 = output.chunk(2, dim=-1) # [B, C, T, F], [B, C, T, F]
        output = output1 * torch.sigmoid(output2) # [B, C, T, F]
...
class FANFFN_gate_channel(nn.Module):
    """
    FANLayer: The layer used in FAN (https://arxiv.org/abs/2410.02675).
    code: https://github.com/YihongDong/FAN/blob/main/FANLayer.py
    Args:
    ...
    def __init__(self, input_dim, expansion):
        super(FANFFN_gate_channel, self).__init__()
    ...
    def forward(self, src):
        ...
        src = F.gelu(src)
        x = self.layernorm(src.permute(0, 2, 3, 1))
        x = self.FAN1(x) # [B, C, T, expansion*F]
        x = self.FAN2(x) # [B, C, T, expansion*F]
        output = self.Linear(x) # [B, C, T, 2F]
        output1, output2 = output.chunk(2, dim=-1) # [B, C, T, F], [B, C, T, F]
        output = output1 * torch.sigmoid(output2) # [B, C, T]
        output = output.permute(0, 3, 1, 2) # [B, C, T, F]
        return output
