...
def validate(
    generator, mssbcqtd, mrd, fn_mel_loss, validset, device, config, rank, world_size, epoch,
):
    generator.eval()
    mssbcqtd.eval()
    mrd.eval()
    ...
    val_batch_size = max(1, int(getattr(env, 'val_batch_size', 2)))
    ...
    totals = {
        'mag': 0.0, 'pha': 0.0, 'com': 0.0, 'con': 0.0, 'gen': 0.0, 'n': 0,
    }
    ref_waveforms = []
    est_waveforms = []
    pbar = None
    ...
    with torch.no_grad():
        for clean_audio, noisy_audio, lengths in loader:
            ...
            totals['con'] += (loss_con.item() / 2) * batch_n
...
def train(config):
    ...
    for epoch in range(start_epoch, env.epochs):
        ...
        val_metrics = validate(
            generator, mssbcqtd, mrd, fn_mel_loss, validset, device, config,
